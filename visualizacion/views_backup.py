...
from equipos.models import Equipo  # Cambiar a usar el modelo correcto
...
def filtrar_datos(request):
    """Vista AJAX para filtrar datos según los criterios seleccionados - SIMPLIFICADA"""
    
    # Solo devolver JSON básico para evitar errores
    return JsonResponse({
        'status': 'success',
    ...
    # Para tabla de equipos: obtener TODOS los equipos
    equipos_individuales = Equipo.objects.all().order_by('unidad_academica__nombre', 'equipo_existente')
    
    # Aplicar filtros a equipos
    if filtros['unidad_academica']:
        equipos_individuales = equipos_individuales.filter(unidad_academica__nombre=filtros['unidad_academica'])
    if filtros['laboratorio']:
        equipos_individuales = equipos_individuales.filter(laboratorio__nombre=filtros['laboratorio'])
    if filtros['estado']:
        equipos_individuales = equipos_individuales.filter(estado=filtros['estado'])
