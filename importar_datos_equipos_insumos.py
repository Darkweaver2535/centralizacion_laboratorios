...
Solo importa equipos e insumos sin dependencias complejas
"""
...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'gestion_laboratorios.settings')
django.setup()
...
def main():
    ...
    print("\n=== IMPORTANDO EQUIPOS ===")
    try:
        df_equipos = pd.read_excel('/Users/alvaroencinas/Desktop/centralizacion_laboratorios/DATOS EQUIPOS.xlsx')
            ...
            try:
                ...
                equipo, created = Equipo.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'marca': '',  # No está en el Excel
                        'modelo': '',  # No está en el Excel
                        'numero_serie': codigo if codigo != 'nan' else '',
                        'estado': estado_activo,
                        'tipo': tipo_general,
                        'unidad_academica': unidad_default,
                        'responsable': usuario_admin,
                        'ubicacion': oficina if oficina != 'nan' else '',
                        'observaciones': f"Responsable: {responsable}, CI: {ci}, Cargo: {cargo}",
                        'activo': True
                    }
                )
    ...
    print("\n=== IMPORTANDO INSUMOS ===")
    try:
        df_insumos = pd.read_excel('/Users/alvaroencinas/Desktop/centralizacion_laboratorios/DATOS INSUMOS.xlsm')
            ...
            try:
                ...
                if categoria_str and categoria_str != 'nan':
                    categoria, _ = CategoriaInsumo.objects.get_or_create(
                        nombre=categoria_str,
                        defaults={'descripcion': f'Categoría {categoria_str}'}
                    )
                else:
                ...
                insumo, created = Insumo.objects.get_or_create(
                    nombre=nombre,
                    defaults={
                        'descripcion': f"Laboratorio: {laboratorio_str}" if laboratorio_str != 'nan' else '',
                        'categoria': categoria,
                        'unidad_academica': unidad_default,
                        'cantidad_disponible': cantidad,
                        'cantidad_minima': 1,
                        'observaciones': f"Unidad origen: {unidad_str}",
                        'activo': True
                    }
                )
