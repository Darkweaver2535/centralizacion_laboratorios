        ...
        try:
            ...
            if not unidad_tematica:
                ...
                unidad_tematica = UnidadTematica.objects.create(
                    asignatura=asignatura,
                    nombre=f"Unidad Temática - {asignatura.nombre}",
                    numero=1,
                    descripcion=f"Unidad temática básica para {asignatura.nombre}"
                )
            ...
            guia, created = GuiaLaboratorio.objects.get_or_create(
                ...
                numero=1,
                defaults={
                    'nombre': f"Guía de Laboratorio - {asignatura.nombre}",
                    'descripcion': f"Guía básica de laboratorio para {asignatura.nombre}"
                }
            )
            ...
            practica, created = Practica.objects.get_or_create(
                ...
                numero=1,
                defaults={
                    'nombre': f"Práctica 1 - {asignatura.nombre}",
                    'descripcion': f"Práctica básica de laboratorio para {asignatura.nombre}"
                }
            )
